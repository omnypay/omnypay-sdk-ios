...
///
/// This class provides a singleton instance that is initialized with a default configuration of <code>ScanConfig
...
/// A shared instance of OmnyPayScan, initialized with default ScanConfig
+ (OmnyPayScan * _Nonnull)sharedInstance;

/// A block that is called when scanning is completed. It takes a ScanResult as parameter and returns nothing.
@property (nonatomic, copy) void (^ _Nullable didScanHandler)(ScanResult * _Nonnull);

/// Configuration of OmnyPayScan instance. It is a read-only property
@property (nonatomic, readonly, strong) ScanConfig * _Nonnull configuration;

/// Creates an instance of OmnyPayScan
///
/// \param configuration An object of <code>ScanConfig
/// </code> type. If nothing is passed in this parameter default scan configuration is used.
///
/// \returns  An instance of <code>OmnyPayScan
/// </code>
...
/// Dismisses the scan view that was presented modally over the view controller passed in presentScanView(over:animated:hadler)
///
/// If <code>scanMode
/// </code> property of receiver object's config (<code>ScanConfig
/// </code>) is <code>Single
/// </code> then scan view is automatically dismissed once a single code is scanned. In all other cases the presenting view controller is responsible for dismissing the scan view it presented.
///
/// \param animated Pass <code>true
...
- (void)dismissScanView:(BOOL)animated completion:(void (^ _Nullable)(void))completion;
...
/// This completion handler takes the following parameters:<code>success
/// </code> - A Boolean to indicate whether scan view is added successfully.<code>error
/// </code> - An NSError object if the scan view is not added, nil otherwise.
...
///
/// If <code>startScanningImmediately
/// </code> property of receiver object's config (<code>ScanConfig
/// </code>) is set to <code>true
/// </code> scanning starts automatically when scan view is presented modally or added as subview, you do not need to call this function.
...
/// If <code>scanMode
/// </code> property of receiver object's config (<code>ScanConfig
/// </code>) is set to <code>Single
/// </code> then scanning automatically stops as soon as a single code is scanned and scan view is dismissed.
///
/// In case of <code>Batch
/// </code> scan mode call this function to stop scanning explicitly.
- (void)stop;
@end
...
/// An instance that represents the configuration of scanner. OmnyPayScan instance can be configured with custom scan mode, code types to scan, whether to start scanning as soon as scanner is presented and a custom dictionary for any other information.
SWIFT_CLASS("_TtC11OmnyPayScan10ScanConfig")
@interface ScanConfig : NSObject

/// A read-only property to get ScanMode value
...
/// A convenience initializer that initialize ScanConfig from another scanconfig object
///
/// \param config A <code>ScanConfig
/// </code> object
///
/// \returns  An instance of <code>ScanConfig
/// </code>
...
/// A type that represents the scanning mode of OmnyPayScan
///
/// <ul><li>Single: Single scan mode. Use this scan mode when a single machine code is to be scanned. Scanner view is dismissed after the scanning completes.</li><li>Batch:  Batch scan mode. Use this scan mode where multiple machine codes are to be scanned.
/// Scanner view continue to scan any code that is detected until <code>stop()
/// </code> is called on <code>OmnyPayScan
/// </code> instance. For each code that is scanned, <code>didScanHandler
/// </code> is executed.
/// Scanner view has to be explicitly closed/removed by calling <code>dismissScanView(animated:, completion:)
/// </code> or <code>removeScanView()
/// </code> on <code>OmnyPayScan
/// </code> instance.</li></ul>
...
/// An object of ScanResult represents the results of scan operation. This object contains a string value of scanned code, a meta type of code and an error if scanning fails.
