...
class ReceiptViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
  ...
  override func viewDidLoad() {
    ...
    self.lblSubTotal.text = "$" + (Double(self.receipt!.summary?.total ?? 0)/100.0).format("%03.2f")
    self.lblTax.text = "$" + (Double(self.receipt!.summary?.tax ?? 0)/100.0).format("%03.2f")
    self.lblDiscount.text = "-$" + (Double(self.receipt!.summary?.discountCents ?? 0)/100.0).format("%03.2f")
    self.lblTotal.text = "$" + (Double(self.receipt!.summary?.paymentTotal ?? 0)/100.0).format("%03.2f")
